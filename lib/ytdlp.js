...
async function getInfo(url, { cookiesFromBrowser } = {}) {
  ...
  for (let attempt = 1; attempt <= INFO_MAX_ATTEMPTS; attempt++) {
    ...
    // YOUTUBE_EXTRACTOR_ARGS phía trên), bỏ hẳn giới hạn client để yt-dlp tự chọn client khác.
    const dropClientRestriction = FORMAT_UNAVAILABLE_ERROR.test(lastRaw);
    const args = [
      url,
      ...JS_RUNTIME_ARGS,
      ...(dropClientRestriction ? [] : YOUTUBE_EXTRACTOR_ARGS),
    ...
    try {
      ...
      lastRaw = raw;
      const isFormatUnavailable = FORMAT_UNAVAILABLE_ERROR.test(raw);
      const canRetry = attempt < INFO_MAX_ATTEMPTS && (TRANSIENT_INFO_ERROR.test(raw) || isFormatUnavailable);
      if (!canRetry) {
        throw new Error(buildErrorMessage(raw, raw || 'Không lấy được thông tin video.'));
      }
      // Lỗi "format not available" nên thử lại ngay với chiến lược client khác, không cần chờ
      // (không phải lỗi mạng tạm thời nên chờ thêm cũng vô ích).
...
function startDownload({ url, type, quality, cookiesFromBrowser }, onProgress) {
  return runDownloadAttempt({ url, type, quality, cookiesFromBrowser }, onProgress, false);
}

// isHardFallback=true: lần thử lại cuối cùng sau khi báo "Requested format is not available"
// — thường do client "web_embedded" (ưu tiên dùng để tránh lỗi 403, xem giải thích ở
// YOUTUBE_EXTRACTOR_ARGS) chỉ trả về storyboard cho video đã tắt tính năng nhúng, nên KHÔNG
// còn định dạng thật nào để chọn dù selector đã có "/best" cuối. Thử lại bỏ hẳn giới hạn
// client (để yt-dlp tự chọn client khác có đủ định dạng) VÀ dùng "-f best" trần không lọc gì,
// để không bao giờ tải thất bại chỉ vì không khớp đúng mức chất lượng yêu cầu.
function runDownloadAttempt({ url, type, quality, cookiesFromBrowser }, onProgress, isHardFallback) {
  ...
  const canHardFallback = !isHardFallback;
  if (type === 'mp3') {
  ...
  return new Promise((resolve, reject) => {
    ...
    const retryHard = () => {
      settled = true;
      cleanupTmp(tmpDir);
      runDownloadAttempt({ url, type, quality, cookiesFromBrowser }, onProgress, true).then(resolve, reject);
    };
    ...
    emitter.on('error', (err) => {
      ...
      if (canHardFallback && FORMAT_UNAVAILABLE_ERROR.test(raw)) return retryHard();
