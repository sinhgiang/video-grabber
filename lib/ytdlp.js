...
async function getInfo(url, { cookiesFromBrowser } = {}) {
  ...
  try {
    ...
    const friendly = classifyYtdlpError(err && err.message) || extractErrorLine(err && err.message);
...
function startDownload({ url, type, quality, cookiesFromBrowser }, onProgress) {
  ...
  return new Promise((resolve, reject) => {
    ...
    emitter.on('error', (err) => {
      ...
      const friendly = classifyYtdlpError(raw) || extractErrorLine(raw);
      reject(friendly ? new Error(friendly) : err);
    });
    emitter.on('close', () => {
      try {
