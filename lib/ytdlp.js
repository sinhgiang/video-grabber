...
async function getInfo(url) {
  const raw = await ytDlpWrap.getVideoInfo([url, ...JS_RUNTIME_ARGS, '-f', 'best']);
 ...
 */
function startDownload({ url, type, quality }, onProgress) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'videograbber-'));
  const outTemplate = path.join(tmpDir, '%(title).150B.%(ext)s');

  const args = [url, '-o', outTemplate, '--ffmpeg-location', ffmpegPath, '--no-playlist', '--newline', ...JS_RUNTIME_ARGS];
...
module.exports = { getInfo, startDownload, cleanupTmp, sanitizeFilename, detectPlatform };
