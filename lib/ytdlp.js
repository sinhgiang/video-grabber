...
// isHardFallback=true: lần thử lại cuối cùng sau khi selector theo chất lượng (kể cả chuỗi
// "/best" cuối) vẫn báo "Requested format is not available" — có thể do danh sách định dạng
// đã thay đổi/khác giữa lúc xem trước và lúc tải thật (hay khác giữa các máy/mạng), hoặc do
// một điểm kỳ lạ trong cách yt-dlp phân giải selector nhiều tầng. Thử lại với "-f best" trần,
// không lọc gì cả, để không bao giờ tải thất bại chỉ vì không khớp đúng mức chất lượng yêu cầu.
function runDownloadAttempt({ url, type, quality, cookiesFromBrowser }, onProgress, isHardFallback) {
  ...
  const canHardFallback = type !== 'mp3' && !isHardFallback;
  if (type === 'mp3') {
